...
void pulls::Loop(TString pdfName)
{
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   */
   ...
   TH1D *huRes = new TH1D("huRes","u resid",100,-1.,1.);
   TH1D *hvPu = new TH1D("hvPu","v pull",100,-6.,6.);
   double momTruth = 1.;
   TH1D *hmomRe = new TH1D("hmomRe","hmomRe",100,0.95*momTruth,1.05*momTruth) ;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      hvpPu->Fill(vpPu);
      huPu->Fill(uPu);
      huRes->Fill(sqrt(pow(uRe-uTr,2.)+pow(vRe-vTr,2.)));
      hvPu->Fill(vPu);
      hfail->Fill(nfail);
      hchi2->Fill(chi2);
   }
