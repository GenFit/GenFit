...
int main() {
  ...
  for (unsigned int iEvent=0; iEvent<nEvents; ++iEvent){
      ...
      try{
    	...
    	//boost::timer::cpu_timer *timer = new boost::timer::cpu_timer();
        kalman.processTrack(fitTrack);
    	//boost::timer::cpu_times elapsed(timer->elapsed());
	//	double elapsed_time = elapsed.user;
...
//  double time_mean = 0;
//  for(int i = 0; i < times.size(); i++) time_mean += times[i];
//  time_mean /= times.size();

//  std::cerr << "EINZELNE LAUFZEIT: " << time_mean << std::endl;
  std::cerr << "OVERALL RUNTIME: " << double((clock() - used_time))/1000000. << std::endl;
