...
void testTrackRep2::setData(const TMatrixT<double>& st, const GFDetPlane& pl, const TMatrixT<double>* cov, const TMatrixT<double>* aux){
  if(aux != NULL) {
    fCacheSpu = (*aux)(0,0);
  } else {
    if(pl!=fCachePlane){
      std::cerr << "testTrackRep2::setData() - a fatal error ocurred! It was called with a reference plane which is not the same as the one from the last extrapolate(plane,state,cov)-> abort in line " << __LINE__ << std::endl;
      throw;
	}
  }
...
const TMatrixT<double>* testTrackRep2::getAuxInfo(const GFDetPlane& pl) {

  Initialize();


  if(pl!=fCachePlane) {
    std::cerr << "testTrackRep2::getAuxInfo() - Fatal error: Trying to get auxillary information with planes mismatch (Information returned does not belong to requested plane)! -> abort in line " << __LINE__ << std::endl;
	throw;
  }
...
testTrackRep2::~testTrackRep2(){
	g4eMgr->CloseGeometry();
}


testTrackRep2::testTrackRep2() : GFAbsTrackRep(5), fDirection(true), fPdg(0), fMass(0.), fCharge(-1), fCachePlane(), fCacheSpu(1), fSpu(1), fAuxInfo(1,1) {
	Initialize();

}


testTrackRep2::testTrackRep2(const TVector3& pos,
                       const TVector3& mom,
...
testTrackRep2::testTrackRep2(const GFTrackCand* aGFTrackCandPtr) :
                       GFAbsTrackRep(5), fDirection(true), fCachePlane(), fCacheSpu(1), fAuxInfo(1,1) {
...
void testTrackRep2::calcStateCov(const TVector3& pos,
                              const TVector3& mom,
                              const TVector3& poserr,
                              const TVector3& momerr){
...
void testTrackRep2::calcState(const TVector3& pos,
                           const TVector3& mom){
...
TMatrixT<double>* testTrackRep2::getState7() const{
...
void testTrackRep2::switchDirection(){
	fDirection = (!fDirection);
	if(fDirection) theG4ErrorMode = G4ErrorMode_PropForwards;
	else theG4ErrorMode = G4ErrorMode_PropBackwards;
}

void testTrackRep2::transformPM(const TMatrixT<double>& in5x5, TMatrixT<double>& out,
...
void testTrackRep2::transformMP(const TMatrixT<double>& in, TMatrixT<double>& out5x5,
...
void testTrackRep2::setPDG(int i){
  fPdg = i;
  TParticlePDG * part = TDatabasePDG::Instance()->GetParticle(fPdg);
  if(part == 0){
    GFException exc("testTrackRep2::setPDG ==> particle id not known to TDatabasePDG",__LINE__,__FILE__);
    throw exc;
  }
  fMass = part->Mass();
...
TVector3 testTrackRep2::getPos(const GFDetPlane& pl){
  if(pl!=fRefPlane){
...
TVector3 testTrackRep2::getMom(const GFDetPlane& pl){
...
void testTrackRep2::getPosMom(const GFDetPlane& pl,TVector3& pos,
                           TVector3& mom){
  TMatrixT<double> statePred(fState);
  if(pl != fRefPlane) {
    extrapolate(pl,statePred);
...
void testTrackRep2::extrapolateToPoint(const TVector3& pos,
				   TVector3& poca,
				   TVector3& dirInPoca){
  ...
  while(true){
    ...
    if(++iterations == maxIt) {
      GFException exc("testTrackRep2::extrapolateToPoint ==> extrapolation to point failed, maximum number of iterations reached",__LINE__,__FILE__);
      throw exc;
    }
  }
...
TVector3 testTrackRep2::poca2Line(const TVector3& extr1,const TVector3& extr2,const TVector3& point) const {
  
  TVector3 theWire = extr2-extr1;
  if(theWire.Mag()<1.E-8){
    GFException exc("testTrackRep2::poca2Line ==> try to find poca between line and point, but the line is really just a point",__LINE__,__FILE__);
    throw exc;
  }
...
void testTrackRep2::extrapolateToLine(const TVector3& point1,
  ...
  while(true){
    ...
    if(fabs(coveredDistance)<MINSTEP) break;
    if(++iterations == maxIt) {
      GFException exc("testTrackRep2::extrapolateToLine ==> extrapolation to line failed, maximum number of iterations reached",__LINE__,__FILE__);
      throw exc;
    }
  }
...
double testTrackRep2::extrapolate(const GFDetPlane& pl,
...
//  
bool testTrackRep2::RKutta (const GFDetPlane& plane,
                         double* P, 
  ...
  if(fabs(fCharge/P[6])<Pmin){
    std::cerr << "testTrackRep2::RKutta ==> momentum too low: " << fabs(fCharge/P[6])*1000. << " MeV" << std::endl;
    return (false);
  }
  ...
  if(fabs(An) < 1.E-6) {
    std::cerr << "testTrackRep2::RKutta ==> cannot propagate perpendicular to plane " << std::endl;
  ...
  if(fabs(Step)>Wmax) {
    std::cerr<<"testTrackRep2::RKutta ==> Too long extrapolation requested : "<<Step<<" cm !"<<std::endl;
  ...
  while(fabs(Step)>MINSTEP && !stopBecauseOfMaterial) {
    ...
    if((Way+=fabs(S))>Wmax){ 
      std::cerr<<"testTrackRep2::RKutta ==> Trajectory is longer than length limit : "<<Way<<" cm !"
  ...
  if(error){
    std::cerr << "testTrackRep2::RKutta ==> Do not get closer. Path = " << Way << " cm" << "  p/q = " << 1./P[6] << " GeV" << std::endl;
    return(false);
  }
...
double testTrackRep2::Extrap( const GFDetPlane& fPlane, TMatrixT<double>* state, TMatrixT<double>* cov)  {
...
void testTrackRep2::getPosMomCov(const GFDetPlane& pl, TVector3& pos, TVector3& mom, TMatrixT<double>& cov6x6){
...
void testTrackRep2::setPosMomCov(const TVector3& pos, const TVector3& mom, const TMatrixT<double>& cov6x6){

  if (cov6x6.GetNcols()!=6 || cov6x6.GetNrows()!=6){
    GFException exc("testTrackRep2::setPosMomCov ==> cov has to be 6x6 (x, y, z, px, py, pz)",__LINE__,__FILE__);
    throw exc;
  }
...
double testTrackRep2::stepalong(double h, TVector3& pos, TVector3& dir){
  ...
  while(true){
    ...
    if(++iterations == maxIt) {
      GFException exc("testTrackRep2::stepalong ==> maximum number of iterations reached",__LINE__,__FILE__);
      throw exc;
    }
  }
